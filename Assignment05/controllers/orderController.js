...
exports.createOrder = (req, res) => {
  const { customer_id, order_date, order_items, total_amount } = req.body
  pool.query(
...
exports.getPercentageOfRepeatCustomers = (req, res) => {
  pool.query(
    'SELECT (SELECT COUNT(DISTINCT customer_id) FROM Orders) as total_customers, (SELECT COUNT(DISTINCT customer_id) FROM Orders GROUP BY customer_id HAVING COUNT(*) > 1) as repeat_customers',
    (error, results) => {
      if (error) {
        return res.status(500).json({ error: error.message })
      }
